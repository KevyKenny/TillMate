...
export async function getAllTimeSummary(userId) {
  ...
  const sumType = async (type) => {
    const r = await db.getFirstAsync(
      `SELECT COALESCE(SUM(amount), 0) AS t FROM finance_transactions WHERE owner_user_id = ? AND type = ?;`,
      [userId, type]
    );
    return Number(r?.t || 0);
  };

  const totalGrossProfitFromSales = await sumType('profit');
  ...
  const salesAgg = await db.getFirstAsync(
    `
      SELECT
        ...
        COALESCE(SUM(COALESCE(p.cost_price, 0) * si.quantity), 0) AS cos,
...
export async function deleteLedgerEntry(userId, entryId) {
  assertUserId(userId);
  const db = await getDb();
  await db.runAsync(`DELETE FROM finance_transactions WHERE id = ? AND owner_user_id = ?;`, [entryId, userId]);
}
...
export async function addStockMovementEntryInTransaction(db, {
  ...
  assertUserId(userId);
  if (!['stock_purchase', 'stock_adjustment'].includes(type)) {
